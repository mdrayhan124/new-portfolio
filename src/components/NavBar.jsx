...
const NavBar = () => {
    ...
    return (
        <div className='flex justify-between items-center text-white bg-black fixed w-full h-20 px-4'>
            <div>
                <h1 className='font-signature text-5xl'>yash</h1>
            </div>
            <ul className='hidden md:flex'>
                {
                    links.map(({ id, link }) => (
                        <li key={id} className='pl-4 text-gray-500 cursor-pointer hover:scale-105 hover:text-white duration-200 capitalize '>{link}</li>
                    ))
                }
            </ul>
            ...
            {
                    links.map(({ id, link }) => (
                        <li key={id} className='px-4 cursor-pointer capitalize text-4xl py-6 hover:text-white'>{link}</li>
                    ))
                }
            </ul>
                )
